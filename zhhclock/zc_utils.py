...
Z_LED_POS = tuple(tuple([(math.sin(idx / ZIPCOUNT * 2 * math.pi),
                          0.0 - math.cos(idx / ZIPCOUNT * 2 * math.pi))
                         for idx in range(ZIPCOUNT)]))


### This is in the display dimensions (-1 to 1)
Z_LED_SPACING = math.pi * ZIP_DIAMETER / ZIPCOUNT / DIM_SCALE
...
for x_idx in range(len(z_leds_near_x)):
    ...
    for zp_idx in height_order_idxs:
        ...
        ### TODO - these may benefit from being in height order which
        ### could be achieved by iterating from 0 to 30 then from 59 to 31 INC
...
def get_pixels_near_angle(angle, radius):
    c_x = math.sin(angle) * ZIP_RADIUS / DIM_SCALE
    c_y = 0.0 - math.cos(angle) * ZIP_RADIUS / DIM_SCALE

    nearest_z_idx = round(angle / (2 * math.pi) * ZIPCOUNT) % ZIPCOUNT
